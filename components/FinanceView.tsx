...
const statusConfig: Record<string, { label: string; color: string; bg: string }> = {
  ...
  processing: { label: 'En cours',   color: 'text-blue-400',    bg: 'bg-blue-500/15' },
  completed:  { label: 'Complété',   color: 'text-emerald-400', bg: 'bg-emerald-500/15' },
  ...
  return (
    <div className="space-y-6 relative">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-black text-white tracking-tighter uppercase italic">General_Ledger_Vault</h2>
          <p className="text-slate-400 font-medium">Rapprochement, gestion des virements et journaux d'audit immuables.</p>
        </div>
        <div className="flex space-x-3">
           <button onClick={() => showToast('Preparing CSV export...')} className="px-4 py-2 bg-slate-900 border border-slate-700 rounded-lg text-sm font-medium text-slate-300 hover:bg-slate-800 flex items-center shadow-sm active:scale-95">
      ...
      {/* Tabs */}
      <div className="flex space-x-1 bg-slate-900 border border-slate-800 p-1 rounded-xl w-fit shadow-inner">
         <button onClick={() => setActiveTab('LEDGER')} className={`px-6 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'LEDGER' ? 'bg-slate-800 text-indigo-400 shadow' : 'text-slate-500 hover:bg-slate-800'}`}>Journal</button>
         <button onClick={() => setActiveTab('PAYOUTS')} className={`px-6 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'PAYOUTS' ? 'bg-slate-800 text-indigo-400 shadow' : 'text-slate-500 hover:bg-slate-800'}`}>Virements</button>
         <button onClick={() => setActiveTab('SETTLEMENTS')} className={`px-6 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'SETTLEMENTS' ? 'bg-slate-800 text-indigo-400 shadow' : 'text-slate-500 hover:bg-slate-800'}`}>Règlements</button>
         <button onClick={() => setActiveTab('RECONCILIATION')} className={`px-6 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'RECONCILIATION' ? 'bg-slate-800 text-indigo-400 shadow' : 'text-slate-500 hover:bg-slate-800'}`}>Rapprochement</button>
      </div>

      {activeTab === 'LEDGER' && (
        <div className="bg-slate-950 rounded-xl border border-slate-800 shadow-sm overflow-hidden animate-in fade-in">
           <div className="px-6 py-4 border-b border-slate-800 flex justify-between items-center bg-slate-900">
              ...
              <div className="relative"><input type="text" placeholder="Rechercher réf..." className="pl-8 pr-3 py-1.5 text-xs border border-slate-700 rounded-lg outline-none focus:ring-2 focus:ring-indigo-500 bg-slate-900 text-white placeholder:text-slate-500" /><Search className="absolute left-2.5 top-2 h-3.5 w-3.5 text-slate-400" /></div>
           </div>
           <table className="min-w-full divide-y divide-slate-800">
              ...
              <tbody className="bg-slate-950 divide-y divide-slate-800">
                 {ledgerEntries.map((entry) => (
                    <tr key={entry.id} onClick={() => setSelectedEntry(entry)} className="hover:bg-indigo-500/10 cursor-pointer transition-colors group">
                       ...
                       <td className="px-6 py-4 text-sm text-white font-medium">{entry.description}</td>
                       <td className="px-6 py-4"><span className={`px-2 py-1 rounded-full text-[9px] font-black uppercase tracking-wider ${entry.category === 'SETTLEMENT' ? 'bg-emerald-500/15 text-emerald-400' : 'bg-slate-900 text-slate-400'}`}>{entry.category}</span></td>
        ...
        <div className="space-y-4 animate-in fade-in">
          ...
          {actionRequiredCount > 0 && (
            <button
              ...
              className="w-full flex items-center gap-3 px-5 py-3.5 bg-red-500/10 border border-red-500/30 rounded-xl text-left hover:bg-red-500/15 transition-colors"
            >
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-red-400 mt-0.5">Retries épuisés — cliquez pour voir les détails</p>
              </div>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {['', 'pending', 'processing', 'completed', 'failed'].map(s => (
              <button
                ...
                className={`px-4 py-1.5 rounded-lg text-xs font-black uppercase tracking-widest border transition-all ${!actionRequiredFilter && statusFilter === s ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-slate-900 text-slate-400 border-slate-700 hover:bg-slate-800'}`}
              >
            ...
            <button
              onClick={() => { setActionRequiredFilter(true); setStatusFilter(''); }}
              className={`px-4 py-1.5 rounded-lg text-xs font-black uppercase tracking-widest border transition-all flex items-center gap-1.5 ${actionRequiredFilter ? 'bg-red-600 text-white border-red-600' : 'bg-slate-900 text-red-400 border-red-500/30 hover:bg-red-500/10'}`}
            >
              <AlertCircle className="w-3 h-3" /> Action requise
              {actionRequiredCount > 0 && <span className={`px-1.5 py-0.5 rounded-full text-[9px] font-black ${actionRequiredFilter ? 'bg-red-400' : 'bg-red-500/20 text-red-400'}`}>{actionRequiredCount}</span>}
            </button>
            ...
            <table className="min-w-full divide-y divide-slate-800">
              ...
              <tbody className="divide-y divide-slate-800">
                {payouts.map((p: any) => {
                  ...
                  return (
                  <tr key={p.id} className={`hover:bg-slate-800/30 transition-colors ${p.requiresAdminAction ? 'bg-red-500/5' : ''}`}>
                    <td className="px-5 py-4">
                      <p className="font-bold text-white text-sm">{p.merchant?.name || '—'}</p>
                      <p className="text-xs text-slate-400 font-mono">{p.merchant?.email}</p>
                    </td>
                    <td className="px-5 py-4">
                      <p className="font-black text-white font-mono">{Number(p.amount).toLocaleString()} {p.currency}</p>
                    </td>
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <p className="text-sm font-medium text-slate-300">{dest}</p>
                    ...
                    <td className="px-5 py-4">
                      <div className="flex flex-col gap-1">
                        ...
                        {p.failureReason && <p className="text-[10px] text-red-400 font-medium">{p.failureReason}</p>}
          ...
          {actionModal && (
            <div className="fixed inset-0 z-[150] bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
              <div className="bg-slate-950 border border-slate-800 rounded-2xl max-w-md w-full shadow-2xl animate-in zoom-in-95">
                <div className="p-6 flex justify-between items-center border-b border-slate-800">
                  <h3 className="text-lg font-bold text-white">
                    {actionModal.type === 'complete' ? 'Confirmer le payout complété' : 'Rejeter le payout'}
                  </h3>
                  <button onClick={() => setActionModal(null)} className="text-slate-400 hover:text-slate-300"><XCircle /></button>
                </div>
                <div className="p-6 space-y-4">
                  {actionModal.type === 'complete' ? (
                    <div>
                      ...
                      <input
                        ...
                        className="w-full p-3 bg-slate-900 border border-slate-700 text-white placeholder:text-slate-500 rounded-xl outline-none font-mono text-sm"
                    ...
                    <div>
                      ...
                      <input
                        value={actionInput}
                        onChange={(e) => setActionInput(e.target.value)}
                        className="w-full p-3 bg-slate-900 border border-slate-700 text-white placeholder:text-slate-500 rounded-xl outline-none text-sm"
                  ...
                  <div className="flex gap-3 pt-2">
                    <button
                      ...
                      className="flex-1 py-3 border border-slate-800 rounded-xl text-sm font-bold text-slate-300 hover:bg-slate-800"
                    >
                      Annuler
                    </button>
                    <button
      ...
      {activeTab === 'SETTLEMENTS' && (
        <div className="space-y-4 animate-in fade-in">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {([
              { value: '',          label: 'Tous' },
              { value: 'pending',   label: 'En transit' },
              { value: 'available', label: 'Libéré' },
              { value: 'settled',   label: 'Versé' },
            ]).map((s) => (
              <button
                key={s.value}
                onClick={() => setSettlementStatusFilter(s.value)}
                className={`px-4 py-1.5 rounded-lg text-xs font-black uppercase tracking-widest border transition-all ${
                  settlementStatusFilter === s.value
                ...
                <tbody className="divide-y divide-slate-800">
                  {settlements.map((s: any) => (
                    <tr key={s.id} className="hover:bg-slate-800/30 transition-colors">
                      <td className="px-5 py-4">
                        <p className="font-bold text-white text-sm">{s.merchantName}</p>
                      ...
                      <td className="px-5 py-4 text-right font-mono text-sm text-slate-300 whitespace-nowrap">
                      ...
                      <td className="px-5 py-4">
                        {s.status === 'settled' ? (
                          <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider bg-indigo-500/15 text-indigo-400">
                            <CheckCircle2 className="w-3 h-3" /> Versé
                          </span>
                        ) : s.status === 'available' ? (
                          <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider bg-emerald-500/15 text-emerald-400">
                            <CheckCircle2 className="w-3 h-3" /> Libéré
                          </span>
                        ) : (
                          <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider bg-amber-500/15 text-amber-400">
      ...
      {activeTab === 'RECONCILIATION' && (
         <div className="space-y-4 animate-in fade-in">
            <div className="grid grid-cols-2 gap-6 h-[500px]">
               <div className="bg-slate-950 rounded-xl border border-slate-800 shadow-sm flex flex-col"><div className="px-4 py-3 border-b border-slate-800 bg-slate-900"><h3 className="font-bold text-slate-300">Relevé bancaire (MTN Escrow)</h3></div><div className="p-2 space-y-2 overflow-y-auto flex-1">{[{id:'TXN-99882',amount:15000000,matched:true}, {id:'TXN-99883',amount:450000,matched:false}].map(tx=>(<div key={tx.id} onClick={()=>!tx.matched && setBankSelection(tx.id)} className={`p-3 border rounded-lg transition-all ${tx.matched ? 'bg-slate-900 opacity-50' : 'cursor-pointer hover:border-indigo-400'} ${bankSelection === tx.id ? 'bg-indigo-500/10 border-indigo-500 shadow-md' : 'border-slate-700'}`}><div className="flex justify-between"><span className="text-sm font-mono font-bold">{tx.id}</span><span className="text-sm font-bold text-green-600">+ {tx.amount.toLocaleString()}</span></div>{tx.matched && <div className="text-xs text-slate-500 mt-1 flex items-center text-green-700"><Check className="h-3 w-3 mr-1"/>Réconcilié</div>}</div>))}</div></div>
               <div className="bg-slate-950 rounded-xl border border-slate-800 shadow-sm flex flex-col"><div className="px-4 py-3 border-b border-slate-800 bg-slate-900"><h3 className="font-bold text-slate-300">Journal interne</h3></div><div className="p-2 space-y-2 overflow-y-auto flex-1">{[{id:'SET-9921',amount:15000000,matched:true}, {id:'SET-9922',amount:450000,matched:false}].map(tx=>(<div key={tx.id} onClick={()=>!tx.matched && setLedgerSelection(tx.id)} className={`p-3 border rounded-lg transition-all ${tx.matched ? 'bg-slate-900 opacity-50' : 'cursor-pointer hover:border-indigo-400'} ${ledgerSelection === tx.id ? 'bg-indigo-500/10 border-indigo-500 shadow-md' : 'border-slate-700'}`}><div className="flex justify-between"><span className="text-sm font-mono font-bold">{tx.id}</span><span className="text-sm font-bold text-indigo-600">{tx.amount.toLocaleString()}</span></div>{tx.matched && <div className="text-xs text-slate-500 mt-1 flex items-center text-green-700"><Check className="h-3 w-3 mr-1"/>Réconcilié</div>}</div>))}</div></div>
            </div>
      ...
      {/* Adjustment Modal */}
      {showAdjustmentModal && (<div className="fixed inset-0 z-[150] bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4"><div className="bg-slate-950 border border-slate-800 rounded-2xl max-w-lg w-full shadow-2xl animate-in zoom-in-95"><div className="p-6 flex justify-between items-center border-b border-slate-800"><h3 className="text-lg font-bold text-white">Créer un ajustement comptable</h3><button onClick={()=>setShowAdjustmentModal(false)} className="text-slate-400 hover:text-slate-300"><XCircle /></button></div><form onSubmit={handleCreateAdjustment} className="p-8 space-y-4"><div><label className="text-sm font-medium text-slate-300">Montant (XOF)</label><input type="number" defaultValue="50000" className="w-full mt-1 p-2 bg-slate-900 border border-slate-700 text-white rounded-lg"/></div><div><label className="text-sm font-medium text-slate-300">Type</label><select className="w-full mt-1 p-2 bg-slate-900 border border-slate-700 text-white rounded-lg"><option>CREDIT</option><option>DEBIT</option></select></div><div><label className="text-sm font-medium text-slate-300">Description</label><textarea placeholder="Ex : Crédit manuel pour équilibrage de flottant" className="w-full mt-1 p-2 bg-slate-900 border border-slate-700 text-white placeholder:text-slate-500 rounded-lg h-24"></textarea></div><button type="submit" className="w-full mt-4 py-3 bg-indigo-600 text-white rounded-lg font-bold hover:bg-indigo-700">Soumettre</button></form></div></div>)}
  {/* Ledger Detail Modal */}
